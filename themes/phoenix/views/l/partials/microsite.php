...
<?php if(!empty(settings()->custom->body_content_microsite)): ?>
    <?= settings()->custom->body_content_microsite ?>
<?php endif ?>
...
<div class="container animate__animated animate__fadeIn <?= isset($_GET['preview']) ? 'container-disabled-simple' : null ?>">
    <?php require THEME_PATH . 'views/l/partials/microsite_scroll_buttons.php' ?>
    <?php require THEME_PATH . 'views/l/partials/microsite_share.php' ?>

    <div class="row d-flex justify-content-center text-center">
        <div class="col-md-<?= $data->link->settings->width ?? '8' ?> link-content">

            <?php require THEME_PATH . 'views/l/partials/ads_header_microsite.php' ?>
                ...
                <div class="row">
                    <?php if($data->link->is_verified): ?>
                        <div id="link-verified-wrapper-top" class="col-12 my-<?= $data->link->settings->block_spacing ?? '2' ?> text-center" style="<?= $data->link->settings->verified_location == 'top' ? null : 'display: none;' ?>">
                            <div>
                                <small class="link-verified" data-toggle="tooltip" title="<?= sprintf(l('link.microsite.verified_help'), settings()->main->title) ?>"><i class="fas fa-fw fa-check-circle fa-1x"></i> <?= l('link.microsite.verified') ?></small>
                            </div>
                        </div>
                    <?php endif ?>

                    <?php if($data->microsite_blocks): ?>
                        <?php
                        /* Detect the location */
                        try {
                        ...
                        <?php foreach($data->microsite_blocks as $row): ?>
            ...
            <?php require THEME_PATH . 'views/l/partials/ads_footer_microsite.php' ?>
...
<?php if(settings()->links->microsites_report_is_enabled): ?>
    <div id="info" class="link-info">
        <a href="<?= url('contact?subject=' . urlencode(sprintf(l('link.microsite.report.subject'), remove_url_protocol_from_url($data->link->full_url))) . '&message=' . urlencode(l('link.microsite.report.message'))) ?>" target="_blank" data-toggle="tooltip" title="<?= l('link.microsite.report') ?>">
...
<script>
    ...
    /* Internal tracking for microsite page blocks */
    document.querySelectorAll('a[data-track-microsite-block-id]').forEach(element => {
        element.addEventListener('click', event => {
            let microsite_block_id = event.currentTarget.getAttribute('data-track-microsite-block-id');
            navigator.sendBeacon(`${site_url}l/link?microsite_block_id=${microsite_block_id}&no_redirect`);
        });
    });
