...
<?php ob_start() ?>
<div class="modal fade" id="<?= 'email_collector_' . $data->link->microsite_block_id ?>" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="<?= 'email_collector_form_' . $data->link->microsite_block_id ?>" method="post" role="form">
                    ...
                    <?php if($data->link->settings->show_agreement): ?>
                        <div class="custom-control custom-switch">
                            <input type="checkbox" id="<?= 'email_collector_agreement_' . $data->link->microsite_block_id ?>" name="agreement" class="custom-control-input" required="required" />
                            <label class="custom-control-label font-weight-normal" for="<?= 'email_collector_agreement_' . $data->link->microsite_block_id ?>">
...
<?php if(!\SeeGap\Event::exists_content_type_key('javascript', 'email_collector')): ?>
    <?php ob_start() ?>
    <script>
        'use strict';
        ...
        $('form[id^="email_collector_"]').each((index, element) => {
            ...
            let is_converted = sessionStorage.getItem(`email_collector_${microsite_block_id}`);
                    ...
                    success: (data) => {
                        ...
                        if(data.status == 'error') {
                            ...
                            setTimeout(() => {
                                ...
                                sessionStorage.setItem(`email_collector_${microsite_block_id}`, true);
    ...
    <?php \SeeGap\Event::add_content(ob_get_clean(), 'javascript', 'email_collector') ?>
<?php endif ?>
