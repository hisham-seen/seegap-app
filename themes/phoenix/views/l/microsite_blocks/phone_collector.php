...
<?php ob_start() ?>
<div class="modal fade" id="<?= 'phone_collector_' . $data->link->microsite_block_id ?>" tabindex="-1" role="dialog" aria-hidden="true">
                ...
                <form id="<?= 'phone_collector_form_' . $data->link->microsite_block_id ?>" method="post" role="form">
                    ...
                    <?php if($data->link->settings->show_agreement): ?>
                        <div class="custom-control custom-switch">
                            <input type="checkbox" id="<?= 'phone_collector_agreement_' . $data->link->microsite_block_id ?>" name="agreement" class="custom-control-input" required="required" />
                            <label class="custom-control-label font-weight-normal" for="<?= 'phone_collector_agreement_' . $data->link->microsite_block_id ?>">
...
<?php if(!\SeeGap\Event::exists_content_type_key('javascript', 'phone_collector')): ?>
    ...
    <script>
        ...
        $('form[id^="phone_collector_"]').each((index, element) => {
            ...
            let is_converted = sessionStorage.getItem(`phone_collector_${microsite_block_id}`);
                    ...
                    success: (data) => {
                        ...
                        if(data.status == 'error') {
                            ...
                            setTimeout(() => {
                                ...
                                sessionStorage.setItem(`phone_collector_${microsite_block_id}`, true);
