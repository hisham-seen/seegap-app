...
<?php if(!isset($_COOKIE['dismiss_close_button_' . $data->link->microsite_block_id])): ?>
<div id="<?= 'microsite_block_id_' . $data->link->microsite_block_id ?>" data-microsite-block-id="<?= $data->link->microsite_block_id ?>" data-microsite-block-type="<?= $data->link->type ?>" class="col-12 my-<?= $data->microsite->settings->block_spacing ?? '2' ?>">
    <div class="alert <?= 'link-btn-' . $data->link->settings->border_radius ?> <?= $data->link->design->link_class ?>" style="<?= $data->link->design->link_style ?>" data-background-color data-border-width data-border-radius data-border-style data-border-color data-border-shadow data-text-alignment data-text-color>
        ...
        <a href="<?= $data->link->location_url . $data->link->utm_query ?>" data-track-microsite-block-id="<?= $data->link->microsite_block_id ?>" target="<?= $data->link->settings->open_in_new_tab ? '_blank' : '_self' ?>" class="text-decoration-none" style="<?= 'color: ' . $data->link->settings->text_color . ';' ?>" data-text-color>
        <?php endif ?>
        ...
        <button type="button" class="close ml-2" data-dismiss="alert" onclick="set_cookie('<?= 'dismiss_close_button_' . $data->link->microsite_block_id ?>', 1, <?= $data->link->settings->alert_pause_after_closed / 60 / 24 ?>, '<?= COOKIE_PATH ?>')"><i class="fas fa-fw fa-sm fa-times" style="<?= 'color: ' . $data->link->settings->text_color . '; opacity: .5;' ?>" data-text-color></i></button>
        <?php endif ?>
    </div>
</div>
<?php endif ?>
