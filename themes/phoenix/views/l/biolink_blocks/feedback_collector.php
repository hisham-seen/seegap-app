...
<div id="<?= 'biolink_block_id_' . $data->link->biolink_block_id ?>" data-biolink-block-id="<?= $data->link->biolink_block_id ?>" data-biolink-block-type="<?= $data->link->type ?>" class="col-12 my-<?= $data->biolink->settings->block_spacing ?? '2' ?>">
    ...
    ?>
    <a href="#" data-toggle="modal" data-target="<?= '#feedback_collector_' . $data->link->biolink_block_id ?>" class="btn btn-block btn-primary link-btn <?= ($data->biolink->settings->hover_animation ?? 'smooth') != 'false' ? 'link-hover-animation-' . ($data->biolink->settings->hover_animation ?? 'smooth') : null ?> <?= 'link-btn-' . $data->link->settings->border_radius ?> <?= $data->link->design->link_class ?> <?= $is_mega_button ? 'mega-button' : '' ?>" style="<?= $data->link->design->link_style ?> <?= $mega_button_style ?>" data-text-color data-border-width data-border-radius data-border-style data-border-color data-border-shadow data-animation data-background-color data-text-alignment>
...
<?php ob_start() ?>
<div class="modal fade" id="<?= 'feedback_collector_' . $data->link->biolink_block_id ?>" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="<?= 'feedback_collector_form_' . $data->link->biolink_block_id ?>" method="post" role="form">
                    ...
                    <?php if($data->link->settings->show_agreement): ?>
                        <div class="custom-control custom-switch">
                            <input type="checkbox" id="<?= 'feedback_collector_agreement_' . $data->link->biolink_block_id ?>" name="agreement" class="custom-control-input" required="required" />
                            <label class="custom-control-label font-weight-normal" for="<?= 'feedback_collector_agreement_' . $data->link->biolink_block_id ?>">
...
<?php if(!\Altum\Event::exists_content_type_key('javascript', 'feedback_collector')): ?>
    ...
    <script>
        ...
        $('form[id^="feedback_collector_"]').each((index, element) => {
            ...
            let is_converted = sessionStorage.getItem(`feedback_collector_${biolink_block_id}`);
                    ...
                    success: (data) => {
                        ...
                        if(data.status == 'error') {
                            ...
                            setTimeout(() => {
                                ...
                                sessionStorage.setItem(`feedback_collector_${biolink_block_id}`, true);
