...
<div class="d-flex flex-column flex-md-row justify-content-between mb-4">
    <h1 class="h3 mb-3 mb-md-0"><i class="fas fa-fw fa-xs fa-table-cells-large text-primary-900 mr-2"></i> <?= l('admin_microsites_blocks.header') ?></h1>
        ...
        <div>
            <div class="dropdown">
                ...
                <div class="dropdown-menu dropdown-menu-right d-print-none">
                    <a href="<?= url('admin/microsites-blocks?' . $data->filters->get_get() . '&export=csv') ?>" target="_blank" class="dropdown-item <?= $this->user->plan_settings->export->csv ? null : 'disabled' ?>">
                        <i class="fas fa-fw fa-sm fa-file-csv mr-2"></i> <?= sprintf(l('global.export_to'), 'CSV') ?>
                    </a>
                    <a href="<?= url('admin/microsites-blocks?' . $data->filters->get_get() . '&export=json') ?>" target="_blank" class="dropdown-item <?= $this->user->plan_settings->export->json ? null : 'disabled' ?>">
                    ...
                    <form action="" method="get" role="form">
                        ...
                        <div class="form-group px-4">
                            ...
                            <select name="type" id="filters_type" class="custom-select custom-select-sm">
                                <option value=""><?= l('global.all') ?></option>
                                <?php foreach($data->microsite_blocks as $key => $value): ?>
                                    <option value="<?= $key ?>" <?= isset($data->filters->filters['type']) && $data->filters->filters['type'] == $key ? 'selected="selected"' : null ?>><?= l('link.microsite.blocks.' . mb_strtolower($key)) ?></option>
                                <?php endforeach ?>
                            </select>
                        </div>
                        ...
                        <div class="form-group px-4">
                            ...
                            <select name="order_by" id="filters_order_by" class="custom-select custom-select-sm">
                                <option value="microsite_block_id" <?= $data->filters->order_by == 'microsite_block_id' ? 'selected="selected"' : null ?>><?= l('global.id') ?></option>
...
<form id="table" action="<?= SITE_URL . 'admin/microsites-blocks/bulk' ?>" method="post" role="form">
    ...
    <table class="table table-custom">
        ...
        <tbody>
        <?php foreach($data->microsites_blocks as $row): ?>
            <?php //SEEGAP:DEMO if(DEMO) {$row->user_email = 'hidden@demo.com'; $row->user_name = 'hidden on demo';} ?>
            <tr>
                <td data-bulk-table class="d-none">
                    <div class="custom-control custom-checkbox">
                        <input id="selected_microsite_block_id_<?= $row->microsite_block_id ?>" type="checkbox" class="custom-control-input" name="selected[]" value="<?= $row->microsite_block_id ?>" />
                        <label class="custom-control-label" for="selected_microsite_block_id_<?= $row->microsite_block_id ?>"></label>
                    </div>
                </td>
                ...
                <td class="text-nowrap">
                    <span><?= $data->microsite_blocks[$row->type]['display_dynamic_name'] ? ($row->settings->{$data->microsite_blocks[$row->type]['display_dynamic_name']} ? string_truncate($row->settings->{$data->microsite_blocks[$row->type]['display_dynamic_name']}, 32) : l('link.microsite.blocks.' . $row->type)) : l('link.microsite.blocks.' . $row->type) ?></span>
                ...
                <td>
                    <div class="d-flex justify-content-end">
                        <?= include_view(THEME_PATH . 'views/admin/microsites-blocks/admin_microsite_block_dropdown_button.php', ['id' => $row->microsite_block_id]) ?>
                    </div>
                </td>
            </tr>
        <?php endforeach ?>
