...
<div class="d-flex flex-column flex-md-row justify-content-between mb-4">
    <h1 class="h3 mb-3 mb-md-0"><i class="fas fa-fw fa-xs fa-moon text-primary-900 mr-2"></i> <?= l('admin_microsites_templates.header') ?></h1>

    <div class="d-flex position-relative d-print-none">
        <div>
            <a href="<?= url('admin/microsite-template-create') ?>" class="btn btn-primary"><i class="fas fa-fw fa-plus-circle fa-sm mr-1"></i> <?= l('admin_microsite_template_create.menu') ?></a>
        </div>
        ...
        <div class="ml-3">
            <div class="dropdown">
                ...
                <div class="dropdown-menu dropdown-menu-right d-print-none">
                    <a href="<?= url('admin/microsites-templates?' . $data->filters->get_get() . '&export=csv') ?>" target="_blank" class="dropdown-item <?= $this->user->plan_settings->export->csv ? null : 'disabled' ?>">
                        <i class="fas fa-fw fa-sm fa-file-csv mr-2"></i> <?= sprintf(l('global.export_to'), 'CSV') ?>
                    </a>
                    <a href="<?= url('admin/microsites-templates?' . $data->filters->get_get() . '&export=json') ?>" target="_blank" class="dropdown-item <?= $this->user->plan_settings->export->json ? null : 'disabled' ?>">
                    ...
                    <form action="" method="get" role="form">
                        ...
                        <div class="form-group px-4">
                            ...
                            <select name="order_by" id="filters_order_by" class="custom-select custom-select-sm">
                                ...
                                <option value="total_usage" <?= $data->filters->order_by == 'total_usage' ? 'selected="selected"' : null ?>><?= l('admin_microsites_templates.main.total_usage') ?></option>
                            </select>
                        </div>
...
<form id="table" action="<?= SITE_URL . 'admin/microsites-templates/bulk' ?>" method="post" role="form">
    ...
    <div class="table-responsive table-custom-container">
        <table class="table table-custom">
            ...
            <tr>
                ...
                <th><?= l('global.name') ?></th>
                <th><?= l('admin_microsites_templates.main.total_usage') ?></th>
            ...
            <tbody>
            <?php foreach($data->microsites_templates as $row): ?>
                <?php //SEEGAP:DEMO if(DEMO) {$row->user_email = 'hidden@demo.com'; $row->user_name = 'hidden on demo';} ?>
                <tr>
                    <td data-bulk-table class="d-none">
                        <div class="custom-control custom-checkbox">
                            <input id="selected_microsite_template_id_<?= $row->microsite_template_id ?>" type="checkbox" class="custom-control-input" name="selected[]" value="<?= $row->microsite_template_id ?>" />
                            <label class="custom-control-label" for="selected_microsite_template_id_<?= $row->microsite_template_id ?>"></label>
                        </div>
                    </td>
                    <td class="text-nowrap">
                        <div>
                            <a href="<?= url('admin/microsite-template-update/' . $row->microsite_template_id) ?>"><?= $row->name ?></a>
                    ...
                    <td>
                        <div class="d-flex justify-content-end">
                            <?= include_view(THEME_PATH . 'views/admin/microsites-templates/admin_microsite_template_dropdown_button.php', ['id' => $row->microsite_template_id, 'resource_name' => $row->name]) ?>
                        </div>
                    </td>
                </tr>
            <?php endforeach ?>
