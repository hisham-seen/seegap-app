...
<div class="dropdown">
    ...
    <div class="dropdown-menu dropdown-menu-right">
        <?php if($data->status === -1 || $data->status == 'uninstalled'): ?>
            <a href="<?= url('admin/plugins/install/' . $data->id . '&global_token=' . \SeeGap\Csrf::get('global_token')) ?>" class="dropdown-item"><i class="fas fa-fw fa-sm fa-plug mr-2"></i> <?= l('admin_plugins.install') ?></a>
        ...
        <?php elseif($data->status === 0 || $data->status == 'installed'): ?>
            <a href="<?= url('admin/plugins/activate/' . $data->id . '&global_token=' . \SeeGap\Csrf::get('global_token')) ?>" class="dropdown-item"><i class="fas fa-fw fa-sm fa-check mr-2"></i> <?= l('admin_plugins.activate') ?></a>
        ...
        <?php elseif($data->status === 1 || $data->status == 'active'): ?>
            <a href="<?= url('admin/plugins/disable/' . $data->id . '&global_token=' . \SeeGap\Csrf::get('global_token')) ?>" class="dropdown-item"><i class="fas fa-fw fa-sm fa-eye-slash mr-2"></i> <?= l('admin_plugins.disable') ?></a>
        <?php endif ?>
