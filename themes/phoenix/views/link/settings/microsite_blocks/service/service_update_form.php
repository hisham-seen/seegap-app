...
<form name="update_microsite_" method="post" role="form">
    ...
    <button class="btn btn-block btn-gray-300 my-4" type="button" data-toggle="collapse" data-target="#<?= 'service_settings_container_' . $row->microsite_block_id ?>" aria-expanded="false" aria-controls="<?= 'service_settings_container_' . $row->microsite_block_id ?>">
        <?= l('microsite_service.service_header') ?>
    </button>

    <div class="collapse" id="<?= 'service_settings_container_' . $row->microsite_block_id ?>">
        <div class="form-group">
            <label for="<?= 'service_title_' . $row->microsite_block_id ?>"><?= l('microsite_service.title') ?></label>
            <input type="text" id="<?= 'service_title_' . $row->microsite_block_id ?>" name="title" class="form-control" value="<?= $row->settings->title ?? null ?>" maxlength="<?= $data->microsite_blocks['service']['fields']['title']['max_length'] ?>" />
        </div>

        <div class="form-group">
            <label for="<?= 'service_description_' . $row->microsite_block_id ?>"><?= l('microsite_service.description') ?></label>
            <input type="text" id="<?= 'service_description_' . $row->microsite_block_id ?>" name="description" class="form-control" value="<?= $row->settings->description ?? null ?>" maxlength="<?= $data->microsite_blocks['service']['fields']['description']['max_length'] ?>" />
        </div>

        <div class="form-group">
            <label for="<?= 'service_price_' . $row->microsite_block_id ?>"><?= l('microsite_product.price') ?></label>
            <input type="number" min="0" step="0.01" id="<?= 'service_price_' . $row->microsite_block_id ?>" name="price" class="form-control" value="<?= $row->settings->price ?? null ?>" />
        </div>

        <div class="form-group">
            <label for="<?= 'service_currency_' . $row->microsite_block_id ?>"><?= l('microsite_service.currency') ?></label>
            <input type="text" id="<?= 'service_currency_' . $row->microsite_block_id ?>" name="currency" class="form-control" value="<?= $row->settings->currency ?? null ?>" maxlength="<?= $data->microsite_blocks['service']['fields']['currency']['max_length'] ?>" />
            <small class="form-text text-muted"><?= l('microsite_service.currency_help') ?></small>
        </div>

        <div class="form-group">
            <label for="<?= 'service_thank_you_title_' . $row->microsite_block_id ?>"><?= l('microsite_donation.thank_you_title') ?></label>
            <input type="text" id="<?= 'service_thank_you_title_' . $row->microsite_block_id ?>" name="thank_you_title" class="form-control" value="<?= $row->settings->thank_you_title ?? null ?>" maxlength="<?= $data->microsite_blocks['service']['fields']['thank_you_title']['max_length'] ?>" />
        </div>

        <div class="form-group">
            <label for="<?= 'service_thank_you_description_' . $row->microsite_block_id ?>"><?= l('microsite_donation.thank_you_description') ?></label>
            <input type="text" id="<?= 'service_thank_you_description_' . $row->microsite_block_id ?>" name="thank_you_description" class="form-control" value="<?= $row->settings->thank_you_description ?? null ?>" maxlength="<?= $data->microsite_blocks['service']['fields']['thank_you_description']['max_length'] ?>" />
        </div>

        <div class="form-group">
            <label for="<?= 'service_thank_you_url_' . $row->microsite_block_id ?>"><?= l('microsite_donation.thank_you_url') ?></label>
            <input type="url" id="<?= 'service_thank_you_url_' . $row->microsite_block_id ?>" name="thank_you_url" class="form-control" value="<?= $row->settings->thank_you_url ?? null ?>" maxlength="<?= $data->microsite_blocks['product']['fields']['thank_you_url']['max_length'] ?>" />
            <small class="form-text text-muted"><?= l('microsite_donation.thank_you_url_help') ?></small>
        </div>

        <div class="mb-3">
            <div class="d-flex flex-column flex-xl-row justify-content-between">
                <label for="<?= 'service_payment_processors_ids_' . $row->microsite_block_id ?>"><?= l('payment_processors.payment_processors_ids') ?></label>
            ...
            <?php foreach($data->payment_processors as $payment_processor): ?>
                <div class="custom-control custom-checkbox my-2">
                    <input id="<?= 'service_payment_processors_ids' . $payment_processor->payment_processor_id . '_' . $row->microsite_block_id ?>" name="payment_processors_ids[]" value="<?= $payment_processor->payment_processor_id ?>" type="checkbox" class="custom-control-input" <?= in_array($payment_processor->payment_processor_id, $row->settings->payment_processors_ids ?? []) ? 'checked="checked"' : null ?>>
                    <label class="custom-control-label" for="<?= 'service_payment_processors_ids' . $payment_processor->payment_processor_id . '_' . $row->microsite_block_id ?>">
    ...
    <button class="btn btn-block btn-gray-300 my-4" type="button" data-toggle="collapse" data-target="#<?= 'service_data_container_' . $row->microsite_block_id ?>" aria-expanded="false" aria-controls="<?= 'service_data_container_' . $row->microsite_block_id ?>">
        <i class="fas fa-fw fa-database fa-sm mr-1"></i> <?= l('microsite_block.data_header') ?>
    </button>

    <div class="collapse" id="<?= 'service_data_container_' . $row->microsite_block_id ?>">
        <div class="alert alert-info">
            <i class="fas fa-fw fa-sm fa-info-circle mr-1"></i> <?= l('microsite_block.data_help') ?>
        </div>

        <div class="form-group">
            <label for="<?= 'service_email_notification_' . $row->microsite_block_id ?>"><?= l('microsite_block.email_notification') ?></label>
            <input type="text" id="<?= 'service_email_notification_' . $row->microsite_block_id ?>" name="email_notification" class="form-control" value="<?= $row->settings->email_notification ?? null ?>" placeholder="<?= l('global.email_placeholder') ?>" maxlength="<?= $data->microsite_blocks['service']['fields']['email_notification']['max_length'] ?>" />
            <small class="form-text text-muted"><?= l('microsite_block.email_notification_help') ?></small>
        </div>

        <div class="form-group">
            <label for="<?= 'service_webhook_url_' . $row->microsite_block_id ?>"><?= l('microsite_block.webhook_url') ?></label>
            <input type="url" id="<?= 'service_webhook_url_' . $row->microsite_block_id ?>" name="webhook_url" class="form-control" value="<?= $row->settings->webhook_url ?? null ?>" placeholder="<?= l('global.url_placeholder') ?>" maxlength="<?= $data->microsite_blocks['service']['fields']['webhook_url']['max_length'] ?>" />
    ...
    <div class="collapse" id="<?= 'button_settings_container_' . $row->microsite_block_id ?>">
        <div class="form-group">
            <label for="<?= 'service_name_' . $row->microsite_block_id ?>"><i class="fas fa-fw fa-signature fa-sm text-muted mr-1"></i> <?= l('microsite_link.name') ?></label>
            <input id="<?= 'service_name_' . $row->microsite_block_id ?>" type="text" name="name" class="form-control" value="<?= $row->settings->name ?>" maxlength="128" required="required" />
        </div>
        ...
        <div class="form-group" data-file-image-input-wrapper data-file-input-wrapper-size-limit="<?= settings()->links->thumbnail_image_size_limit ?>" data-file-input-wrapper-size-limit-error="<?= sprintf(l('global.error_message.file_size_limit'), settings()->links->thumbnail_image_size_limit) ?>">
            <label for="<?= 'service_image_' . $row->microsite_block_id ?>"><i class="fas fa-fw fa-image fa-sm text-muted mr-1"></i> <?= l('microsite_link.image') ?></label>
            <?= include_view(THEME_PATH . 'views/partials/custom_file_image_input.php', [
                'id'=> 'service_image_' . $row->microsite_block_id,
                'uploads_file_key' => 'block_thumbnail_images',
                'file_key' => 'image',
                'already_existing_image' => $row->settings->image,
                'image_container' => 'image',
                'accept' => \SeeGap\Uploads::array_to_list_format($data->microsite_blocks['service']['whitelisted_thumbnail_image_extensions']),
            ]) ?>
            <small class="form-text text-muted"><?= sprintf(l('global.accessibility.whitelisted_file_extensions'), \SeeGap\Uploads::array_to_list_format($data->microsite_blocks['service']['whitelisted_thumbnail_image_extensions'])) . ' ' . sprintf(l('global.accessibility.file_size_limit'), settings()->links->thumbnail_image_size_limit) ?></small>
        </div>

        <div class="form-group">
            <label for="<?= 'service_icon_' . $row->microsite_block_id ?>"><i class="fas fa-fw fa-icons fa-sm text-muted mr-1"></i> <?= l('global.icon') ?></label>
            <input id="<?= 'service_icon_' . $row->microsite_block_id ?>" type="text" name="icon" class="form-control" value="<?= $row->settings->icon ?>" placeholder="<?= l('global.icon_placeholder') ?>" />
            <small class="form-text text-muted"><?= l('global.icon_help') ?></small>
        </div>

        <div class="form-group">
            <label for="<?= 'service_text_color_' . $row->microsite_block_id ?>"><i class="fas fa-fw fa-paint-brush fa-sm text-muted mr-1"></i> <?= l('microsite_link.text_color') ?></label>
            <input id="<?= 'service_text_color_' . $row->microsite_block_id ?>" type="hidden" name="text_color" class="form-control" value="<?= $row->settings->text_color ?>" required="required" />
        ...
        <div class="form-group">
            <label for="<?= 'service_background_color_' . $row->microsite_block_id ?>"><i class="fas fa-fw fa-fill fa-sm text-muted mr-1"></i> <?= l('microsite_link.background_color') ?></label>
            <input id="<?= 'service_background_color_' . $row->microsite_block_id ?>" type="hidden" name="background_color" class="form-control" value="<?= $row->settings->background_color ?>" required="required" />
            <div class="background_color_pickr"></div>
        </div>

        <div class="form-group">
            <label for="<?= 'service_animation_' . $row->microsite_block_id ?>"><i class="fas fa-fw fa-film fa-sm text-muted mr-1"></i> <?= l('microsite_link.animation') ?></label>
            <select id="<?= 'service_animation_' . $row->microsite_block_id ?>" name="animation" class="custom-select">
        ...
        <div class="form-group" data-animation="<?= implode(',', require APP_PATH . 'includes/microsite_animations.php') ?>">
            <label for="<?= 'service_animation_runs_' . $row->microsite_block_id ?>"><i class="fas fa-fw fa-play-circle fa-sm text-muted mr-1"></i> <?= l('microsite_link.animation_runs') ?></label>
            <select id="<?= 'service_animation_runs_' . $row->microsite_block_id ?>" name="animation_runs" class="custom-select">
