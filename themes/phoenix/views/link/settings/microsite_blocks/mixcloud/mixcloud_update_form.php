...
<form name="update_microsite_" method="post" role="form">
    ...
    <div class="form-group">
        <label for="<?= 'mixcloud_location_url_' . $row->microsite_block_id ?>"><i class="fas fa-fw fa-link fa-sm text-muted mr-1"></i> <?= l('microsite_mixcloud.location_url') ?></label>
        <input id="<?= 'mixcloud_location_url_' . $row->microsite_block_id ?>" type="text" class="form-control" name="location_url" value="<?= $row->location_url ?>" maxlength="2048" placeholder="<?= l('microsite_mixcloud.location_url_placeholder') ?>" required="required" />
    </div>

    <div class="form-group">
        <label for="<?= 'mixcloud_type_' . $row->microsite_block_id ?>"><i class="fas fa-fw fa-fingerprint fa-sm text-muted mr-1"></i> <?= l('global.type') ?></label>
        <select id="<?= 'mixcloud_type_' . $row->microsite_block_id ?>" name="type" class="custom-select">
            <option value="classic" <?= $row->settings->theme == 'classic' ? 'selected="selected"' : null ?>><?= l('microsite_mixcloud.type.classic') ?></option>
            <option value="mini" <?= $row->settings->theme == 'mini' ? 'selected="selected"' : null ?>><?= l('microsite_mixcloud.type.mini') ?></option>
            <option value="picture" <?= $row->settings->theme == 'picture' ? 'selected="selected"' : null ?>><?= l('microsite_mixcloud.type.picture') ?></option>
        </select>
    </div>

    <div class="form-group">
        <label for="<?= 'mixcloud_theme_' . $row->microsite_block_id ?>"><i class="fas fa-fw fa-sun fa-sm text-muted mr-1"></i> <?= l('microsite_mixcloud.theme') ?></label>
        <select id="<?= 'mixcloud_theme_' . $row->microsite_block_id ?>" name="theme" class="custom-select">
