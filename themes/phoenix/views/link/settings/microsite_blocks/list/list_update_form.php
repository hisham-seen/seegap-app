...
<form name="update_microsite_" method="post" role="form">
    ...
    <div class="form-group">
        <label for="<?= 'list_type_' . $row->microsite_block_id ?>"><i class="fas fa-fw fa-list fa-sm text-muted mr-1"></i> <?= l('microsite_list.list_type') ?></label>
        <div class="row btn-group-toggle" data-toggle="buttons">
            <div class="col-6">
                <label class="btn btn-light btn-block text-truncate <?= ($row->settings->list_type ?? 'unordered') == 'unordered' ? 'active' : '' ?>">
                    ...
                    <i class="fas fa-fw fa-list-ul fa-sm mr-1"></i> <?= l('microsite_list.list_type_unordered') ?>
                </label>
            </div>
            <div class="col-6">
                <label class="btn btn-light btn-block text-truncate <?= ($row->settings->list_type ?? 'unordered') == 'ordered' ? 'active' : '' ?>">
                    ...
                    <i class="fas fa-fw fa-list-ol fa-sm mr-1"></i> <?= l('microsite_list.list_type_ordered') ?>
                </label>
            </div>
        </div>
    </div>
    ...
    <div class="form-group">
        <label><i class="fas fa-fw fa-list fa-sm text-muted mr-1"></i> <?= l('microsite_list.list_items') ?></label>
        <div id="<?= 'list_items_container_' . $row->microsite_block_id ?>">
                ...
                <?php foreach($row->settings->list_items as $key => $list_item): ?>
                    <div class="list-item-wrapper mb-2">
                        <div class="input-group">
                            <input type="text" name="list_items[]" class="form-control" value="<?= htmlspecialchars($list_item) ?>" placeholder="<?= l('microsite_list.list_item_placeholder') ?>" maxlength="256" />
            ...
            <?php else: ?>
                <div class="list-item-wrapper mb-2">
                    <div class="input-group">
                        <input type="text" name="list_items[]" class="form-control" value="" placeholder="<?= l('microsite_list.list_item_placeholder') ?>" maxlength="256" />
                        <div class="input-group-append">
                            <button type="button" class="btn btn-outline-danger remove-list-item">
                                <i class="fas fa-times fa-sm"></i>
                            </button>
        ...
        <button type="button" class="btn btn-sm btn-outline-primary mt-2" id="<?= 'add_list_item_' . $row->microsite_block_id ?>">
            <i class="fas fa-plus fa-sm mr-1"></i> <?= l('microsite_list.add_list_item') ?>
        </button>
        <small class="form-text text-muted"><?= l('microsite_list.list_items_help') ?></small>
    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    addButton.addEventListener('click', function() {
        ...
        if (itemCount >= 100) {
            alert('<?= l('microsite_list.max_items_reached') ?>');
            return;
        }
