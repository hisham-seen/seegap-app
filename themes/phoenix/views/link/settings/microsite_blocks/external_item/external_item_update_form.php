...
<form name="update_microsite_" method="post" role="form">
    ...
    <div class="form-group">
        <label for="<?= 'external_item_name_' . $row->microsite_block_id ?>"><i class="fas fa-fw fa-signature fa-sm text-muted mr-1"></i> <?= l('microsite_link.name') ?></label>
        <input id="<?= 'external_item_name_' . $row->microsite_block_id ?>" type="text" name="name" class="form-control" value="<?= $row->settings->name ?>" maxlength="128" required="required" />
    </div>

    <div class="form-group">
        <label for="<?= 'external_item_description_' . $row->microsite_block_id ?>"><i class="fas fa-fw fa-paragraph fa-sm text-muted mr-1"></i> <?= l('microsite_link.description') ?></label>
        <input id="<?= 'external_item_description_' . $row->microsite_block_id ?>" type="text" name="description" class="form-control" value="<?= $row->settings->description ?>" maxlength="256" required="required" />
    </div>

    <div class="form-group">
        <label for="<?= 'external_item_price_' . $row->microsite_block_id ?>"><i class="fas fa-fw fa-dollar-sign fa-sm text-muted mr-1"></i> <?= l('microsite_external_item.price') ?></label>
        <input id="<?= 'external_item_price_' . $row->microsite_block_id ?>" type="text" name="price" class="form-control" value="<?= $row->settings->price ?>" maxlength="32" placeholder="<?= l('microsite_external_item.price_placeholder') ?>" required="required" />
    </div>

    <div class="form-group">
        <label for="<?= 'external_item_location_url_' . $row->microsite_block_id ?>"><i class="fas fa-fw fa-link fa-sm text-muted mr-1"></i> <?= l('microsite_external_item.location_url') ?></label>
        <input id="<?= 'external_item_location_url_' . $row->microsite_block_id ?>" type="text" class="form-control" name="location_url" value="<?= $row->location_url ?>" maxlength="2048" placeholder="<?= l('global.url_placeholder') ?>" />
    </div>

    <div class="form-group custom-control custom-switch">
        <input
                id="<?= 'external_item_open_in_new_tab_' . $row->microsite_block_id ?>"
                name="open_in_new_tab" type="checkbox"
                class="custom-control-input"
            <?= $row->settings->open_in_new_tab ? 'checked="checked"' : null ?>
        >
        <label class="custom-control-label" for="<?= 'external_item_open_in_new_tab_' . $row->microsite_block_id ?>"><?= l('microsite_link.open_in_new_tab') ?></label>
    </div>
    ...
    <div class="form-group" data-file-image-input-wrapper data-file-input-wrapper-size-limit="<?= settings()->links->thumbnail_image_size_limit ?>" data-file-input-wrapper-size-limit-error="<?= sprintf(l('global.error_message.file_size_limit'), settings()->links->thumbnail_image_size_limit) ?>">
        <label for="<?= 'external_item_image_' . $row->microsite_block_id ?>"><i class="fas fa-fw fa-image fa-sm text-muted mr-1"></i> <?= l('microsite_link.image') ?></label>
        <?= include_view(THEME_PATH . 'views/partials/custom_file_image_input.php', [
            'id'=> 'external_item_image_' . $row->microsite_block_id,
            'uploads_file_key' => 'block_thumbnail_images',
            'file_key' => 'image',
            'already_existing_image' => $row->settings->image,
            'image_container' => 'image',
            'accept' => \SeeGap\Uploads::array_to_list_format($data->microsite_blocks['external_item']['whitelisted_thumbnail_image_extensions']),
        ]) ?>
        <small class="form-text text-muted"><?= sprintf(l('global.accessibility.whitelisted_file_extensions'), \SeeGap\Uploads::array_to_list_format($data->microsite_blocks['external_item']['whitelisted_thumbnail_image_extensions'])) . ' ' . sprintf(l('global.accessibility.file_size_limit'), settings()->links->thumbnail_image_size_limit) ?></small>
    </div>
    ...
    <div class="collapse" id="<?= 'button_settings_container_' . $row->microsite_block_id ?>">
        <div class="form-group">
            <label for="<?= 'external_item_name_color_' . $row->microsite_block_id ?>"><i class="fas fa-fw fa-paint-brush fa-sm text-muted mr-1"></i> <?= l('microsite_external_item.name_color') ?></label>
            <input id="<?= 'external_item_name_color_' . $row->microsite_block_id ?>" type="hidden" name="name_color" class="form-control" value="<?= $row->settings->name_color ?>" required="required" />
        ...
        <div class="form-group">
            <label for="<?= 'external_item_description_color_' . $row->microsite_block_id ?>"><i class="fas fa-fw fa-paint-brush fa-sm text-muted mr-1"></i> <?= l('microsite_link.description_color') ?></label>
            <input id="<?= 'external_item_description_color_' . $row->microsite_block_id ?>" type="hidden" name="description_color" class="form-control" value="<?= $row->settings->description_color ?>" required="required" />
            <div class="description_color_pickr"></div>
        </div>

        <div class="form-group">
            <label for="<?= 'external_item_price_color_' . $row->microsite_block_id ?>"><i class="fas fa-fw fa-paint-brush fa-sm text-muted mr-1"></i> <?= l('microsite_external_item.price_color') ?></label>
            <input id="<?= 'external_item_price_color_' . $row->microsite_block_id ?>" type="hidden" name="price_color" class="form-control" value="<?= $row->settings->price_color ?>" required="required" />
            <div class="price_color_pickr"></div>
        </div>

        <div class="form-group">
            <label for="<?= 'external_item_background_color_' . $row->microsite_block_id ?>"><i class="fas fa-fw fa-fill fa-sm text-muted mr-1"></i> <?= l('microsite_link.background_color') ?></label>
            <input id="<?= 'external_item_background_color_' . $row->microsite_block_id ?>" type="hidden" name="background_color" class="form-control" value="<?= $row->settings->background_color ?>" required="required" />
            <div class="background_color_pickr"></div>
        </div>

        <div class="form-group">
            <label for="<?= 'external_item_animation_' . $row->microsite_block_id ?>"><i class="fas fa-fw fa-film fa-sm text-muted mr-1"></i> <?= l('microsite_link.animation') ?></label>
            <select id="<?= 'external_item_animation_' . $row->microsite_block_id ?>" name="animation" class="custom-select">
        ...
        <div class="form-group" data-animation="<?= implode(',', require APP_PATH . 'includes/microsite_animations.php') ?>">
            <label for="<?= 'external_item_animation_runs_' . $row->microsite_block_id ?>"><i class="fas fa-fw fa-play-circle fa-sm text-muted mr-1"></i> <?= l('microsite_link.animation_runs') ?></label>
            <select id="<?= 'external_item_animation_runs_' . $row->microsite_block_id ?>" name="animation_runs" class="custom-select">
