...
# Install wheel explicitly with a specific version to avoid issues
echo "Installing wheel 0.45.0..."
pip install wheel==0.45.0

# Choose whether to use pip cache
if $USE_CACHE; then
  echo "Using pip cache..."
  # Try with uv first, fall back to pip if it fails
  uv pip install "${ARGS[@]}" || pip install "${ARGS[@]}"
else
  echo "Disabling pip cache..."
  # Try with uv first, fall back to pip if it fails
  uv pip install --no-cache-dir "${ARGS[@]}" || pip install --no-cache-dir "${ARGS[@]}"
fi
