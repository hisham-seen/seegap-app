...
<div id="<?= 'microsite_block_id_' . $data->link->microsite_block_id ?>" data-microsite-block-id="<?= $data->link->microsite_block_id ?>" data-microsite-block-type="<?= $data->link->type ?>" class="col-12 my-<?= $data->microsite->settings->block_spacing ?? '2' ?>">
    <div class="d-flex align-items-center justify-content-center">
        <div id="<?= 'flipdown_' . $data->link->microsite_block_id ?>" class="flipdown" data-end-date="<?= (new DateTime($data->link->settings->counter_end_date))->getTimestamp() ?>"></div>
    </div>
</div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        new FlipDown(
            parseInt(document.querySelector('#<?= 'flipdown_' . $data->link->microsite_block_id ?>').getAttribute('data-end-date')),
            '<?= 'flipdown_' . $data->link->microsite_block_id ?>',
            {
