...
<div class="card mb-5">
    <div class="card-body">
        <div class="d-flex justify-content-between mb-4">
            <h2 class="h4 text-truncate mb-0"><i class="<?= $data->microsite_blocks[$key]['icon'] ?> fa-xs text-primary-900 mr-2"></i> <?= l('link.microsite.blocks.' . $key) ?></h2>
...
<script>
    ...
    new Chart(<?= $key ?>_chart, {
        type: 'line',
        data: {
            labels: <?= $data->microsites_blocks_chart[$key]['labels'] ?>,
            datasets: [
                {
                    label: <?= json_encode(l('link.microsite.blocks.' . $key)) ?>,
                    data: <?= $data->microsites_blocks_chart[$key]['total'] ?? '[]' ?>,
