...
<div class="row">
    <div class="col-12 col-xl-6">
        ...
        <?php
        $active_tab = settings()->links->microsites_default_active_tab ?? 'settings';
        ...
        <div class="d-flex flex-column flex-lg-row justify-content-lg-between  mb-4">
            <ul class="nav nav-pills microsite-switch-buttons mb-3 mb-lg-0" id="pills-tab" role="tablist">
                <li class="nav-item">
                ...
                <li class="nav-item">
                    <a class="nav-link <?= $active_tab == 'blocks' ? 'active' : null ?>" id="blocks-tab" data-toggle="pill" href="#microsite_blocks" role="tab" aria-controls="links" aria-selected="false">
            ...
            <div>
                <button type="button" data-toggle="modal" data-target="#microsite_link_create_modal" class="btn btn-primary"><i class="fas fa-fw fa-plus-circle fa-sm mr-1"></i> <?= l('links.create_microsite_block') ?></button>
            </div>
        </div>
        ...
        <div class="tab-content">
                        ...
                        <form id="update_microsite" name="update_microsite" action="" method="post" role="form" enctype="multipart/form-data">
                            ...
                            <?php if(settings()->links->microsites_themes_is_enabled): ?>
                                ...
                                <div class="collapse" id="theme_container" data-parent="#settings">
                                    <div class="form-group">
                                        <label><i class="fas fa-fw fa-palette fa-sm text-muted mr-1"></i> <?= l('link.settings.microsite_theme_id') ?></label>
                                        ...
                                        <div class="position-relative">
                                            ...
                                            <div id="microsites_themes" class="microsite-themes-wrapper d-flex" style="overflow-x: scroll; width: 100%;">
                                                <?php foreach($data->microsites_themes as $key => $theme): ?>
                                                    ...
                                                    <label for="settings_microsite_theme_id_<?= $key ?>" class="m-0 col-6 p-3" <?= in_array($theme->microsite_theme_id, $this->user->plan_settings->microsites_themes ?? []) ? null : 'data-toggle="tooltip" title="' . l('global.info_message.plan_feature_no_access') . '"' ?>>
                                                ...
                                                <label for="settings_microsite_theme_id_null" class="m-0 col-6 p-3">
            ...
            <div class="tab-pane fade <?= $active_tab == 'blocks' ? 'show active' : null ?>" id="microsite_blocks" role="tabpanel" aria-labelledby="blocks-tab">
                ...
                <?php if($data->link_links_result->num_rows): ?>
                    <?php while($row = $data->link_links_result->fetch_object()): ?>
                        ...
                        <div class="microsite_block card <?= $row->is_enabled ? null : 'custom-row-inactive' ?> mb-4" data-microsite-block-id="<?= $row->microsite_block_id ?>">
                            <div class="card-body">
                                <div class="d-flex align-items-center">
                                    ...
                                    <div class="col-1 mr-2 p-0 d-none d-lg-block">
                                        <span class="fa-stack fa-1x" data-toggle="tooltip" title="<?= l('link.microsite.blocks.' . $row->type) ?>">
                                            <i class="fas fa-circle fa-stack-2x" style="color: <?= $data->microsite_blocks[$row->type]['color'] ?>"></i>
                                            <i class="<?= $data->microsite_blocks[$row->type]['icon'] ?> fa-stack-1x fa-inverse"></i>
                                        </span>
                                    </div>
                                        ...
                                        <div class="d-flex flex-column text-truncate">
                                            <div class="text-truncate">
                                                <a href="#"
                                                   ...
                                                   class="text-truncate"
                                                >
                                                    <strong><?= $data->microsite_blocks[$row->type]['display_dynamic_name'] ? ($row->settings->{$data->microsite_blocks[$row->type]['display_dynamic_name']} ? string_truncate($row->settings->{$data->microsite_blocks[$row->type]['display_dynamic_name']}, 32) : l('link.microsite.blocks.' . $row->type)) : l('link.microsite.blocks.' . $row->type) ?></strong>
                                                </a>
                                            </div>
                                    ...
                                    <div class="d-none d-md-flex col-md-3 justify-content-end flex-wrap">
                                        <?php if($data->microsite_blocks[$row->type]['has_statistics']): ?>
                                            <a href="<?= url('microsite-block/' . $row->microsite_block_id . '/statistics') ?>">
                                        ...
                                        <?php if($data->microsite_blocks[$row->type]['type'] == 'payment'): ?>
                                            <a href="<?= url('guests-payments?microsite_block_id=' . $row->microsite_block_id) ?>" class="btn btn-sm btn-link text-secondary" data-toggle="tooltip" title="<?= l('guests_payments.link') ?>">
                                                <i class="fas fa-fw fa-sm fa-coins"></i>
                                            </a>
                                            <a href="<?= url('guests-payments-statistics?microsite_block_id=' . $row->microsite_block_id) ?>" class="btn btn-sm btn-link text-secondary" data-toggle="tooltip" title="<?= l('guests_payments_statistics.link') ?>">
                                    ...
                                    <div class="col-5 col-md d-flex align-items-center justify-content-end">
                                        <div class="custom-control custom-switch" data-toggle="tooltip" title="<?= l('link.microsite_blocks.is_enabled_tooltip') ?>">
                                            <input
                                                    type="checkbox"
                                                    class="custom-control-input"
...
<template id="template_vcard_social">
    <div class="mb-4">
        <div class="form-group">
            ...
            <input id="" type="text" name="vcard_social_label[]" class="form-control" maxlength="<?= $data->microsite_blocks['vcard']['fields']['social_label']['max_length'] ?>" required="required" />
        </div>

        <div class="form-group">
            <label for=""><i class="fas fa-fw fa-link fa-sm text-muted mr-1"></i> <?= l('microsite_vcard.vcard_social_value') ?></label>
            <input id="" type="url" name="vcard_social_value[]" class="form-control" maxlength="<?= $data->microsite_blocks['vcard']['fields']['social_value']['max_length'] ?>" required="required" />
        </div>
...
<script>
    ...
    $('form[name="update_microsite"],form[name="update_microsite_"]').on('submit', event => {
