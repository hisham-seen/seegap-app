                ...
                <?php foreach(require APP_PATH . 'includes/microsite_blocks_categories.php' as $microsite_block_category_key => $microsite_block_category): ?>
                    ...
                    <?php foreach(require APP_PATH . 'includes/enabled_microsite_blocks.php' as $key => $value): ?>
                        ...
                        <?php if($this->user->plan_settings->enabled_microsite_blocks->{$key}): ?>
                            <div class="col-12 col-lg-6 p-3" data-block-category="<?= $value['category'] ?>" data-block-id="<?= $key ?>" data-block-name="<?= l('link.microsite.blocks.' . $key) ?>">
                                <button
                                    type="button"
                                    data-dismiss="modal"
                                    data-toggle="modal"
                                ...
                                >
                                    <span class="fa-stack fa-stack-small mr-2">
                                        <i class="fas fa-circle fa-stack-2x" style="color: <?= $data->microsite_blocks[$key]['color'] ?>"></i>
                                        <i class="<?= $data->microsite_blocks[$key]['icon'] ?> fa-stack-1x fa-inverse"></i>
                                    </span>

                                    <?= l('link.microsite.blocks.' . $key) ?>
                                </button>
                            </div>
                        ...
                        <?php else: ?>
                            <div class="col-12 col-lg-6 p-3" data-block-category="<?= $value['category'] ?>" data-block-id="<?= $key ?>" data-block-name="<?= l('link.microsite.blocks.' . $key) ?>">
                                ...
                                >
                                    ...
                                    <s><?= l('link.microsite.blocks.' . $key) ?></s>
                                </button>
                            </div>
                    ...
                    <?php if($enabled_blocks_html || $disabled_blocks_html): ?>
                        <div class="mb-4" data-category="<?= $microsite_block_category_key ?>">
                            <div class="card text-white border-0 mb-3" style="background: <?= $microsite_block_category['color'] ?>">
                                <div class="card-body d-flex justify-content-between align-items-center">
                                    <div>
                                        <span class="h6"><?= l('microsite_link_create.' . $microsite_block_category_key) ?></span>
                                        <p class="small mb-0"><?= l('microsite_link_create.' . $microsite_block_category_key . '_subheader') ?></p>
                                    </div>

                                    <div>
                                        <i class="fas fa-fw fa-lg <?= $microsite_block_category['icon'] ?>"></i>
                                    </div>
                                </div>
                            </div>
