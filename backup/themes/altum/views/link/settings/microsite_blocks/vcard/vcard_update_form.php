...
<form name="update_microsite_" method="post" role="form">
    ...
    <button class="btn btn-block btn-gray-300 my-4" type="button" data-toggle="collapse" data-target="#<?= 'vcard_settings_container_' . $row->microsite_block_id ?>" aria-expanded="false" aria-controls="<?= 'vcard_settings_container_' . $row->microsite_block_id ?>">
        <i class="fas fa-fw fa-id-card fa-sm mr-1"></i>  <?= l('microsite_vcard.vcard_header') ?>
    </button>

    <div class="collapse" id="<?= 'vcard_settings_container_' . $row->microsite_block_id ?>">
        <div class="form-group">
            <label for="<?= 'vcard_avatar_' . $row->microsite_block_id ?>"><i class="fas fa-fw fa-image fa-sm text-muted mr-1"></i> <?= l('microsite_vcard.vcard_avatar') ?></label>
            <div data-image-container="vcard_avatar" class="<?= !empty($row->settings->vcard_avatar) ? null : 'd-none' ?>">
                ...
                <div class="custom-control custom-checkbox my-2">
                    <input id="<?= $row->microsite_block_id . '_vcard_avatar_remove' ?>" name="vcard_avatar_remove" type="checkbox" class="custom-control-input" onchange="this.checked ? document.querySelector('#<?= 'vcard_avatar_' . $row->microsite_block_id ?>').classList.add('d-none') : document.querySelector('#<?= 'vcard_avatar_' . $row->microsite_block_id ?>').classList.remove('d-none')">
                    <label class="custom-control-label" for="<?= $row->microsite_block_id . '_vcard_avatar_remove' ?>">
                        <span class="text-muted"><?= l('global.delete_file') ?></span>
                    </label>
                </div>
            </div>
            <input id="<?= 'vcard_avatar_' . $row->microsite_block_id ?>" type="file" name="vcard_avatar" accept="<?= \Altum\Uploads::get_whitelisted_file_extensions_accept('vcards_avatars') ?>" class="form-control-file altum-file-input" />
        ...
        <div class="row">
            <div class="col-6">
                <div class="form-group">
                    <label for="<?= 'vcard_first_name_' . $row->microsite_block_id ?>"><i class="fas fa-fw fa-signature fa-sm text-muted mr-1"></i> <?= l('microsite_vcard.vcard_first_name') ?></label>
                    <input type="text" id="<?= 'vcard_first_name_' . $row->microsite_block_id ?>" name="vcard_first_name" class="form-control" value="<?= $row->settings->vcard_first_name ?? null ?>" maxlength="<?= $data->microsite_blocks['vcard']['fields']['first_name']['max_length'] ?>" />
                </div>
            </div>
            <div class="col-6">
                <div class="form-group">
                    <label for="<?= 'vcard_last_name_' . $row->microsite_block_id ?>"><i class="fas fa-fw fa-signature fa-sm text-muted mr-1"></i> <?= l('microsite_vcard.vcard_last_name') ?></label>
                    <input type="text" id="<?= 'vcard_last_name_' . $row->microsite_block_id ?>" name="vcard_last_name" class="form-control" value="<?= $row->settings->vcard_last_name ?? null ?>" maxlength="<?= $data->microsite_blocks['vcard']['fields']['last_name']['max_length'] ?>" />
                </div>
            </div>
        </div>

        <div class="form-group">
            <label for="<?= 'vcard_email_' . $row->microsite_block_id ?>"><i class="fas fa-fw fa-envelope fa-sm text-muted mr-1"></i> <?= l('global.email') ?></label>
            <input type="email" id="<?= 'vcard_email_' . $row->microsite_block_id ?>" name="vcard_email" class="form-control" value="<?= $row->settings->vcard_email ?? null ?>" maxlength="<?= $data->microsite_blocks['vcard']['fields']['email']['max_length'] ?>" />
        </div>

        <div class="form-group">
            <label for="<?= 'vcard_url_' . $row->microsite_block_id ?>"><i class="fas fa-fw fa-link fa-sm text-muted mr-1"></i> <?= l('microsite_vcard.vcard_url') ?></label>
            <input type="url" id="<?= 'vcard_url_' . $row->microsite_block_id ?>" name="vcard_url" class="form-control" value="<?= $row->settings->vcard_url ?? null ?>" maxlength="<?= $data->microsite_blocks['vcard']['fields']['url']['max_length'] ?>" />
        </div>

        <div class="form-group">
            <label for="<?= 'vcard_company_' . $row->microsite_block_id ?>"><i class="fas fa-fw fa-building fa-sm text-muted mr-1"></i> <?= l('microsite_vcard.vcard_company') ?></label>
            <input type="text" id="<?= 'vcard_company_' . $row->microsite_block_id ?>" name="vcard_company" class="form-control" value="<?= $row->settings->vcard_company ?? null ?>" maxlength="<?= $data->microsite_blocks['vcard']['fields']['company']['max_length'] ?>" />
        </div>

        <div class="form-group">
            <label for="<?= 'vcard_job_title_' . $row->microsite_block_id ?>"><i class="fas fa-fw fa-user-tie fa-sm text-muted mr-1"></i> <?= l('microsite_vcard.vcard_job_title') ?></label>
            <input type="text" id="<?= 'vcard_job_title_' . $row->microsite_block_id ?>" name="vcard_job_title" class="form-control" value="<?= $row->settings->vcard_job_title ?? null ?>" maxlength="<?= $data->microsite_blocks['vcard']['fields']['job_title']['max_length'] ?>" />
        </div>

        <div class="form-group">
            <label for="<?= 'vcard_birthday_' . $row->microsite_block_id ?>"><i class="fas fa-fw fa-birthday-cake fa-sm text-muted mr-1"></i> <?= l('microsite_vcard.vcard_birthday') ?></label>
            <input type="date" id="<?= 'vcard_birthday_' . $row->microsite_block_id ?>" name="vcard_birthday" class="form-control" value="<?= $row->settings->vcard_birthday ?? null ?>" />
        </div>

        <div class="form-group">
            <label for="<?= 'vcard_street_' . $row->microsite_block_id ?>"><i class="fas fa-fw fa-road fa-sm text-muted mr-1"></i> <?= l('microsite_vcard.vcard_street') ?></label>
            <input type="text" id="<?= 'vcard_street_' . $row->microsite_block_id ?>" name="vcard_street" class="form-control" value="<?= $row->settings->vcard_street ?? null ?>" maxlength="<?= $data->microsite_blocks['vcard']['fields']['street']['max_length'] ?>" />
        </div>

        <div class="form-group">
            <label for="<?= 'vcard_city_' . $row->microsite_block_id ?>"><i class="fas fa-fw fa-city fa-sm text-muted mr-1"></i> <?= l('global.city') ?></label>
            <input type="text" id="<?= 'vcard_city_' . $row->microsite_block_id ?>" name="vcard_city" class="form-control" value="<?= $row->settings->vcard_city ?? null ?>" maxlength="<?= $data->microsite_blocks['vcard']['fields']['city']['max_length'] ?>" />
        </div>

        <div class="form-group">
            <label for="<?= 'vcard_zip_' . $row->microsite_block_id ?>"><i class="fas fa-fw fa-mail-bulk fa-sm text-muted mr-1"></i> <?= l('microsite_vcard.vcard_zip') ?></label>
            <input type="text" id="<?= 'vcard_zip_' . $row->microsite_block_id ?>" name="vcard_zip" class="form-control" value="<?= $row->settings->vcard_zip ?? null ?>" maxlength="<?= $data->microsite_blocks['vcard']['fields']['zip']['max_length'] ?>" />
        </div>

        <div class="form-group">
            <label for="<?= 'vcard_region_' . $row->microsite_block_id ?>"><i class="fas fa-fw fa-flag fa-sm text-muted mr-1"></i> <?= l('microsite_vcard.vcard_region') ?></label>
            <input type="text" id="<?= 'vcard_region_' . $row->microsite_block_id ?>" name="vcard_region" class="form-control" value="<?= $row->settings->vcard_region ?? null ?>" maxlength="<?= $data->microsite_blocks['vcard']['fields']['region']['max_length'] ?>" />
        </div>

        <div class="form-group">
            <label for="<?= 'vcard_country_' . $row->microsite_block_id ?>"><i class="fas fa-fw fa-globe fa-sm text-muted mr-1"></i> <?= l('global.country') ?></label>
            <input type="text" id="<?= 'vcard_country_' . $row->microsite_block_id ?>" name="vcard_country" class="form-control" value="<?= $row->settings->vcard_country ?? null ?>" maxlength="<?= $data->microsite_blocks['vcard']['fields']['country']['max_length'] ?>" />
        </div>

        <div class="form-group">
            <label for="<?= 'vcard_note_' . $row->microsite_block_id ?>"><i class="fas fa-fw fa-paragraph fa-sm text-muted mr-1"></i> <?= l('microsite_vcard.vcard_note') ?></label>
            <textarea id="<?= 'vcard_note_' . $row->microsite_block_id ?>" name="vcard_note" class="form-control" maxlength="<?= $data->microsite_blocks['vcard']['fields']['note']['max_length'] ?>"><?= $row->settings->vcard_note ?? null ?></textarea>
        </div>

        <button class="btn btn-block btn-gray-300 my-4" type="button" data-toggle="collapse" data-target="#<?= 'vcard_phone_numbers_container_' . $row->microsite_block_id ?>" aria-expanded="false" aria-controls="<?= 'vcard_phone_numbers_container_' . $row->microsite_block_id ?>">
            <i class="fas fa-fw fa-phone-square-alt fa-sm mr-1"></i> <?= l('microsite_vcard.vcard_phone_numbers') ?>
        </button>

        <div class="collapse" id="<?= 'vcard_phone_numbers_container_' . $row->microsite_block_id ?>">
            <div id="<?= 'vcard_phone_numbers_' . $row->microsite_block_id ?>" data-microsite-block-id="<?= $row->microsite_block_id ?>">
                <?php foreach($row->settings->vcard_phone_numbers ?? [] as $key => $phone_number): ?>
                    <div class="mb-4">
                        <div class="form-group">
                            <label for="<?= 'vcard_phone_number_label_' . $key . '_' . $row->microsite_block_id ?>"><i class="fas fa-fw fa-bookmark fa-sm text-muted mr-1"></i> <?= l('microsite_vcard.vcard_phone_number_label') ?></label>
                            <input id="<?= 'vcard_phone_number_label_' . $key . '_' . $row->microsite_block_id ?>" type="text" name="vcard_phone_number_label[<?= $key ?>]" class="form-control" value="<?= $phone_number->label ?>" maxlength="<?= $data->microsite_blocks['vcard']['fields']['phone_number_label']['max_length'] ?>" />
                        </div>

                        <div class="form-group">
                            <label for="<?= 'vcard_phone_number_value_' . $key . '_' . $row->microsite_block_id ?>"><i class="fas fa-fw fa-phone-square-alt fa-sm text-muted mr-1"></i> <?= l('microsite_vcard.vcard_phone_number_value') ?></label>
                            <input id="<?= 'vcard_phone_number_value_' . $key . '_' . $row->microsite_block_id ?>" type="text" name="vcard_phone_number_value[<?= $key ?>]" value="<?= $phone_number->value ?>" class="form-control" maxlength="<?= $data->microsite_blocks['vcard']['fields']['phone_number_value']['max_length'] ?>" required="required" />
                        </div>
            ...
            <div class="mb-3">
                <button data-add="vcard_phone_numbers" data-microsite-block-id="<?= $row->microsite_block_id ?>" type="button" class="btn btn-outline-success btn-block"><i class="fas fa-fw fa-plus-circle fa-sm mr-1"></i> <?= l('global.create') ?></button>
            </div>
        </div>

        <button class="btn btn-block btn-gray-300 my-4" type="button" data-toggle="collapse" data-target="#<?= 'vcard_socials_container_' . $row->microsite_block_id ?>" aria-expanded="false" aria-controls="<?= 'vcard_socials_container_' . $row->microsite_block_id ?>">
            <i class="fas fa-fw fa-share-alt fa-sm mr-1"></i> <?= l('microsite_vcard.vcard_socials') ?>
        </button>

        <div class="collapse" id="<?= 'vcard_socials_container_' . $row->microsite_block_id ?>">
            <div id="<?= 'vcard_socials_' . $row->microsite_block_id ?>" data-microsite-block-id="<?= $row->microsite_block_id ?>">
                <?php foreach($row->settings->vcard_socials ?? [] as $key => $social): ?>
                    <div class="mb-4">
                        <div class="form-group">
                            <label for="<?= 'vcard_social_label_' . $key . '_' . $row->microsite_block_id ?>"><i class="fas fa-fw fa-bookmark fa-sm text-muted mr-1"></i> <?= l('microsite_vcard.vcard_social_label') ?></label>
                            <input id="<?= 'vcard_social_label_' . $key . '_' . $row->microsite_block_id ?>" type="text" name="vcard_social_label[<?= $key ?>]" class="form-control" value="<?= $social->label ?>" maxlength="<?= $data->microsite_blocks['vcard']['fields']['social_label']['max_length'] ?>" required="required" />
                        </div>

                        <div class="form-group">
                            <label for="<?= 'vcard_social_value_' . $key . '_' . $row->microsite_block_id ?>"><i class="fas fa-fw fa-link fa-sm text-muted mr-1"></i> <?= l('microsite_vcard.vcard_social_value') ?></label>
                            <input id="<?= 'vcard_social_value_' . $key . '_' . $row->microsite_block_id ?>" type="url" name="vcard_social_value[<?= $key ?>]" value="<?= $social->value ?>" class="form-control" maxlength="<?= $data->microsite_blocks['vcard']['fields']['social_value']['max_length'] ?>" required="required" />
                        </div>
            ...
            <div class="mb-3">
                <button data-add="vcard_social" data-microsite-block-id="<?= $row->microsite_block_id ?>" type="button" class="btn btn-outline-success btn-block"><i class="fas fa-fw fa-plus-circle fa-sm mr-1"></i> <?= l('global.create') ?></button>
            </div>
        </div>
    </div>
    ...
    <div class="collapse" id="<?= 'button_settings_container_' . $row->microsite_block_id ?>">
        <div class="form-group">
            <label for="<?= 'vcard_name_' . $row->microsite_block_id ?>"><i class="fas fa-fw fa-signature fa-sm text-muted mr-1"></i> <?= l('microsite_link.name') ?></label>
            <input id="<?= 'vcard_name_' . $row->microsite_block_id ?>" type="text" name="name" class="form-control" value="<?= $row->settings->name ?>" maxlength="128" required="required" />
        </div>
        ...
        <div class="form-group" data-file-image-input-wrapper data-file-input-wrapper-size-limit="<?= settings()->links->thumbnail_image_size_limit ?>" data-file-input-wrapper-size-limit-error="<?= sprintf(l('global.error_message.file_size_limit'), settings()->links->thumbnail_image_size_limit) ?>">
            <label for="<?= 'vcard_image_' . $row->microsite_block_id ?>"><i class="fas fa-fw fa-image fa-sm text-muted mr-1"></i> <?= l('microsite_link.image') ?></label>
            <?= include_view(THEME_PATH . 'views/partials/custom_file_image_input.php', [
                'id'=> 'vcard_image_' . $row->microsite_block_id,
                'uploads_file_key' => 'block_thumbnail_images',
                'file_key' => 'image',
                'already_existing_image' => $row->settings->image,
                'image_container' => 'image',
                'accept' => \Altum\Uploads::array_to_list_format($data->microsite_blocks['vcard']['whitelisted_thumbnail_image_extensions']),
            ]) ?>
            <small class="form-text text-muted"><?= sprintf(l('global.accessibility.whitelisted_file_extensions'), \Altum\Uploads::array_to_list_format($data->microsite_blocks['vcard']['whitelisted_thumbnail_image_extensions'])) . ' ' . sprintf(l('global.accessibility.file_size_limit'), settings()->links->thumbnail_image_size_limit) ?></small>
        </div>

        <div class="form-group">
            <label for="<?= 'vcard_icon_' . $row->microsite_block_id ?>"><i class="fas fa-fw fa-icons fa-sm text-muted mr-1"></i> <?= l('global.icon') ?></label>
            <input id="<?= 'vcard_icon_' . $row->microsite_block_id ?>" type="text" name="icon" class="form-control" value="<?= $row->settings->icon ?>" placeholder="<?= l('global.icon_placeholder') ?>" />
            <small class="form-text text-muted"><?= l('global.icon_help') ?></small>
        </div>

        <div class="form-group">
            <label for="<?= 'vcard_text_color_' . $row->microsite_block_id ?>"><i class="fas fa-fw fa-paint-brush fa-sm text-muted mr-1"></i> <?= l('microsite_link.text_color') ?></label>
            <input id="<?= 'vcard_text_color_' . $row->microsite_block_id ?>" type="hidden" name="text_color" class="form-control" value="<?= $row->settings->text_color ?>" required="required" />
        ...
        <div class="form-group">
            <label for="<?= 'vcard_background_color_' . $row->microsite_block_id ?>"><i class="fas fa-fw fa-fill fa-sm text-muted mr-1"></i> <?= l('microsite_link.background_color') ?></label>
            <input id="<?= 'vcard_background_color_' . $row->microsite_block_id ?>" type="hidden" name="background_color" class="form-control" value="<?= $row->settings->background_color ?>" required="required" />
            <div class="background_color_pickr"></div>
        </div>

        <div class="form-group">
            <label for="<?= 'vcard_animation_' . $row->microsite_block_id ?>"><i class="fas fa-fw fa-film fa-sm text-muted mr-1"></i> <?= l('microsite_link.animation') ?></label>
            <select id="<?= 'vcard_animation_' . $row->microsite_block_id ?>" name="animation" class="custom-select">
        ...
        <div class="form-group" data-animation="<?= implode(',', require APP_PATH . 'includes/microsite_animations.php') ?>">
            <label for="<?= 'vcard_animation_runs_' . $row->microsite_block_id ?>"><i class="fas fa-fw fa-play-circle fa-sm text-muted mr-1"></i> <?= l('microsite_link.animation_runs') ?></label>
            <select id="<?= 'vcard_animation_runs_' . $row->microsite_block_id ?>" name="animation_runs" class="custom-select">
