...
class BiolinkBlockAjax extends Controller {
    ...
    private function create() {
        ...
        if(isset($_POST['block_type']) && array_key_exists($_POST['block_type'], $this->biolink_blocks)) {
            ...
            $individual_blocks = ['link', 'heading', 'big_link', 'paragraph', 'markdown', 'avatar', 'socials', 'email_collector', 'custom_html', 'image', 'image_grid', 'divider', 'list', 'alert', 'faq', 'timeline', 'review', 'image_slider', 'countdown', 'cta', 'external_item', 'share', 'coupon', 'youtube_feed', 'paypal', 'phone_collector', 'contact_collector', 'feedback_collector', 'donation', 'product', 'service', 'map', 'iframe', 'header'];
            $embeddable_blocks = ['telegram', 'anchor', 'applemusic', 'soundcloud', 'threads', 'snapchat', 'spotify', 'tidal', 'mixcloud', 'kick', 'tiktok_video', 'typeform', 'calendly', 'tiktok_profile', 'twitch', 'twitter_tweet', 'twitter_video', 'twitter_profile', 'pinterest_profile', 'vimeo', 'youtube', 'instagram_media', 'facebook', 'reddit', 'rumble'];
    ...
    private function create_biolink_feedback_collector() {
        $_POST['link_id'] = (int) $_POST['link_id'];
        $_POST['name'] = mb_substr(query_clean($_POST['name']), 0, 128);
        $_POST['image_display'] = in_array($_POST['image_display'], ['icon', 'background', 'card', 'full_background', 'mega_button']) ? query_clean($_POST['image_display']) : 'icon';
        $_POST['button_size'] = in_array($_POST['button_size'], ['xs', 'sm', 'md', 'lg', 'xl', 'xxl']) ? query_clean($_POST['button_size']) : 'md';
        $_POST['hide_text'] = (int) isset($_POST['hide_text']);
        $_POST['image_fit'] = in_array($_POST['image_fit'], ['contain', 'cover', 'fill', 'scale-down']) ? query_clean($_POST['image_fit']) : 'contain';
        $_POST['icon_shape'] = in_array($_POST['icon_shape'], ['round', 'rounded', 'square']) ? query_clean($_POST['icon_shape']) : 'rounded';
        $_POST['icon_position'] = in_array($_POST['icon_position'], ['left', 'right']) ? query_clean($_POST['icon_position']) : 'left';
        $_POST['mega_button_height'] = in_array($_POST['mega_button_height'], range(100, 500)) ? (int) $_POST['mega_button_height'] : 200;
        $_POST['show_text'] = (int) isset($_POST['show_text']);
        ...
        if(isset($_POST['question_text']) && is_array($_POST['question_text']) && 
           isset($_POST['question_type']) && is_array($_POST['question_type'])) {
            
            foreach($_POST['question_text'] as $index => $question_text) {
                if(empty(trim($question_text))) continue;
                
                $question_type = $_POST['question_type'][$index] ?? 'text';
                $question_required = isset($_POST['question_required'][$index]) ? true : false;
                
                $question = [
                    'question' => input_clean($question_text, 256),
                    'type' => $question_type,
                ...
                if(in_array($question_type, ['rating_star', 'rating_number'])) {
                    $question['options']->max_rating = isset($_POST['question_max_rating'][$index]) ? (int) $_POST['question_max_rating'][$index] : 5;
                }
                
                if(in_array($question_type, ['checkbox', 'radio', 'dropdown'])) {
                    $choices = isset($_POST['question_choices'][$index]) ? explode("\n", $_POST['question_choices'][$index]) : [];
        ...
        $settings = json_encode([
            ...
            'hide_text' => $_POST['hide_text'],
            'icon_shape' => $_POST['icon_shape'],
            'icon_position' => $_POST['icon_position'],
            'phone_placeholder' => l('biolink_feedback_collector.phone_placeholder_default'),
            'name_placeholder' => l('biolink_feedback_collector.name_placeholder_default'),
            'message_placeholder' => l('biolink_feedback_collector.message_placeholder_default'),
            'email_placeholder' => l('biolink_feedback_collector.email_placeholder_default'),
            'button_text' => l('biolink_feedback_collector.button_text_default'),
            'success_text' => l('biolink_feedback_collector.success_text_default'),
            'thank_you_url' => '',
            'show_agreement' => false,
            'agreement_url' => '',
            'agreement_text' => '',
    ...
    private function update_biolink_feedback_collector() {
        $_POST['biolink_block_id'] = (int) $_POST['biolink_block_id'];
        $_POST['name'] = mb_substr(query_clean($_POST['name']), 0, 128);
        $_POST['border_radius'] = in_array($_POST['border_radius'], ['straight', 'round', 'rounded']) ? query_clean($_POST['border_radius']) : 'rounded';
        $_POST['border_width'] = in_array($_POST['border_width'], [0, 1, 2, 3, 4, 5]) ? (int) $_POST['border_width'] : 0;
