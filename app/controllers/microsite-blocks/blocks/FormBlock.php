            ...
            if(isset($_POST['question_text']) && is_array($_POST['question_text'])) {
                foreach($_POST['question_text'] as $key => $question_text) {
                    if(!empty(trim($question_text))) {
                        ...
                        $question->type = in_array($_POST['question_type'][$key] ?? 'text', ['text', 'textarea', 'email', 'phone', 'rating_star', 'rating_number', 'rating_emoji', 'checkbox', 'radio', 'dropdown']) ? $_POST['question_type'][$key] : 'text';
