...
class Link extends Controller {
    ...
    public function microsite_block_ajax() {
        if(empty($_POST)) {
            die();
        }
    ...
    private function submit_form() {
        ...
        $microsite_block = db()->where('microsite_block_id', $microsite_block_id)->getOne('microsites_blocks');
        ...
        $form_data['submitted_at'] = get_date();
        $form_data['ip'] = get_ip();
        ...
        try {
            ...
            if($form_submission_id) {
                ...
                Response::json($response_data['message'], 'success', $response_data['details'] ?? []);
            } else {
                Response::json(l('global.error_message.basic'), 'error');
            }
