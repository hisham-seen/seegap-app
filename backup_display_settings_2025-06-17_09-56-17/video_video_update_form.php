...
<form name="update_microsite_" method="post" role="form" enctype="multipart/form-data">
    ...
    <div class="form-group">
        <label for="<?= 'video_file_' . $row->microsite_block_id ?>"><i class="fas fa-fw fa-file fa-sm text-muted mr-1"></i> <?= l('microsite_file.file') ?></label>
        <div class="row">
            <div class="col">
                <input id="<?= 'video_file_' . $row->microsite_block_id ?>" type="file" name="file" accept="<?= \SeeGap\Uploads::array_to_list_format($data->microsite_blocks['video']['whitelisted_file_extensions']) ?>" class="form-control-file seegap-file-input" />
            </div>
        ...
        <small class="form-text text-muted"><?= sprintf(l('global.accessibility.whitelisted_file_extensions'), \SeeGap\Uploads::array_to_list_format($data->microsite_blocks['video']['whitelisted_file_extensions'])) . ' ' . sprintf(l('global.accessibility.file_size_limit'), settings()->links->file_size_limit) ?></small>
    </div>

    <div class="form-group">
        <label for="<?= 'video_poster_url_' . $row->microsite_block_id ?>"><i class="fas fa-fw fa-image fa-sm text-muted mr-1"></i> <?= l('microsite_video.poster_url') ?></label>
        <input id="<?= 'video_poster_url_' . $row->microsite_block_id ?>" type="url" name="poster_url" maxlength="2048" class="form-control" value="<?= $row->settings->poster_url ?>" />
    </div>

    <div class="form-group">
        <label for="<?= 'video_name_' . $row->microsite_block_id ?>"><i class="fas fa-fw fa-signature fa-sm text-muted mr-1"></i> <?= l('microsite_link.name') ?></label>
        <input id="<?= 'video_name_' . $row->microsite_block_id ?>" type="text" name="name" class="form-control" value="<?= $row->settings->name ?>" maxlength="128" required="required" />
    </div>
    ...
    <div class="form-group custom-control custom-switch">
        <input
                ...
                id="<?= 'video_autoplay_' . $row->microsite_block_id ?>"
                name="video_autoplay"
            <?= $row->settings->video_autoplay ? 'checked="checked"' : null ?>
        >
        <label class="custom-control-label" for="<?= 'video_autoplay_' . $row->microsite_block_id ?>"><?= l('microsite_video.video_autoplay') ?></label>
    </div>
    ...
    <div class="form-group custom-control custom-switch">
        <input
                type="checkbox"
                class="custom-control-input"
                id="<?= 'video_controls_' . $row->microsite_block_id ?>"
                name="video_controls"
            <?= $row->settings->video_controls ? 'checked="checked"' : null ?>
        >
        <label class="custom-control-label" for="<?= 'video_controls_' . $row->microsite_block_id ?>"><?= l('microsite_video.video_controls') ?></label>
    </div>
    ...
    <div class="form-group custom-control custom-switch">
        <input
                type="checkbox"
                class="custom-control-input"
                id="<?= 'video_loop_' . $row->microsite_block_id ?>"
                name="video_loop"
            <?= $row->settings->video_loop ? 'checked="checked"' : null ?>
        >
        <label class="custom-control-label" for="<?= 'video_loop_' . $row->microsite_block_id ?>"><?= l('microsite_video.video_loop') ?></label>
    </div>
    ...
    <div class="form-group custom-control custom-switch">
        <input
                type="checkbox"
                class="custom-control-input"
                id="<?= 'video_muted_' . $row->microsite_block_id ?>"
                name="video_muted"
            <?= $row->settings->video_muted ? 'checked="checked"' : null ?>
        >
        <label class="custom-control-label" for="<?= 'video_muted_' . $row->microsite_block_id ?>"><?= l('microsite_video.video_muted') ?></label>
    </div>
